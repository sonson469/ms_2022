...
public class SaveManager
{
	...
	private class SaveDataBase
	{
		...
		public T GetClass<T>(string key, T _default) where T : class
		{
			keyCheck(key);
			if (!saveDictionary.ContainsKey(key))
				return _default;
		...
		public void SetClass<T>(string key, T obj) where T : class
		{
			keyCheck(key);
