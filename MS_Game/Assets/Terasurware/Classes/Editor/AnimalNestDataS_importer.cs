...
public class AnimalNestDataS_importer : AssetPostprocessor {
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		foreach (string asset in importedAssets) {
			...
			AnimalNestData data = (AnimalNestData)AssetDatabase.LoadAssetAtPath (exportPath, typeof(AnimalNestData));
			if (data == null) {
				data = ScriptableObject.CreateInstance<AnimalNestData> ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					AnimalNestData.Sheet s = new AnimalNestData.Sheet ();
					s.name = sheetName;
					...
					for (int i=1; i<= sheet.LastRowNum; i++) {
						...
						AnimalNestData.Param p = new AnimalNestData.Param ();
